...
{
#pragma omp for schedule(dynamic, 8) nowait // Циклическое распределение итераций
    for (int i = 0; i < n - 1; i++)
    {
        for (int j = i + 1; j < n; j++)
        {
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < n; i++)
    {
        p[i].x = rand() / RAND_MAX - 0.5;
        p[i].y = rand() / RAND_MAX - 0.5;
        p[i].z = rand() / RAND_MAX - 0.5;
        v[i].x = rand() / RAND_MAX - 0.5;
        v[i].y = rand() / RAND_MAX - 0.5;
        v[i].z = rand() / RAND_MAX - 0.5;
        m[i] = rand() / RAND_MAX * 10 + 0.01;
        f[i].x = f[i].y = f[i].z = 0;
    }
    tinit += omp_get_wtime();
    double dt = 1e-5;
